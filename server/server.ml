...
let traitement_sortie player= 
	...
	let rec remove l =
		match l with
			h :: t -> if h.id = player.id then remove t else (output_string h.outchan (sorti player.nom)); h::(remove t)
			| [] -> []
	in joueur_liste := remove !joueur_liste;
	Mutex.unlock mutex_joueurL;;
...
let boucle_joueur player =
	...
	while true do
		...
		let (num_req, args) = received_request line in
			...
			| 3 -> 1 (* todo*)
			| 4 -> 1 (* todo*)
			| _ ->output_string outch "REQUETE INVALIDE : "^line; flush player.outchan
	done;
	with Exit->true;;
...
let joueur_service sock=
	...
	let (num_req, args) = received_request line in
		if num_req = 1 then
			let player = (creer_joueur (car args) sock inchan outchan) in
			traitement_connecte player;
			boucle_joueur player
		else
			output_string outchan "REQUETE INVALIDE: attente d'une requete de la forme: CONNEXION/username"; flush outchan
